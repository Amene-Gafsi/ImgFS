...
{ // Check if arguments are valid
    if (resolution != THUMB_RES && resolution != SMALL_RES && resolution != ORIG_RES)
        return ERR_INVALID_ARGUMENT; // grader: if resolution is not valid, you should return ERR_RESOLUTIONS,
        // except for ORIG_RES which should be ERR_NONE -0.5
    ...
    if (!imgfs_file->metadata[index].is_valid) // grader: check relies on the fact that IS_VALID is 1, use the enum value, -0.5
        return ERR_INVALID_IMGID;
    ...
    // grader: you should check if orig_img is NULL, -1
    ...
    if (vips_thumbnail_image(vips_orig_img, &vips_resized_img, width, NULL))
        // grader: missing other arguments, works because our images are square, but wouldn't work if we want to resize
        // to non-square images, -1
    {
        free(orig_img);
    ...
    {
        // grader: you should write only one element of the metadata array, not the whole array, -1
        free_images(orig_img, resized_img, vips_orig_img, vips_resized_img);
        return ERR_IO;
    }
