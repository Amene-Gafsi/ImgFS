...
int do_delete(const char *img_id, struct imgfs_file *imgfs_file)
{
    ...
    if (metadata == NULL)
    {
        ...
        //grader: this check should be done before memcpy, mempcpy(NULL,...,...) is undefined behavior, -0.5
    }

    // grader: you are modifying the copied metadata in case of success: there is no need to do a copy of the metadata -1

    for (uint32_t i = 0; i < imgfs_file->header.max_files; i++)
    {
        if (!strcmp(imgfs_file->metadata[i].img_id, img_id))
        {
            //grader: you should first check if the image is valid (O(1) comparison) before comparing the name (O(n)), -0.5
    ...
    {
        //grader: you are supposed to write only one element of the metadata array, not the whole array -1
