...
int do_create(const char *imgfs_filename, struct imgfs_file *imgfs_file)
{
    ...
    //grader : warning: missing is_valid init. Is it initialized with the calloc (since it sets memory to 0), otherwise if EMPTY
    //wasn't 0, this wouldn't initialize it properly
    ...
    if (imgfs == NULL)
        // grader: you should free the allocated metadata before returning, -1
    ...
    {
        // grader: you should free the allocated metadata before returning, not further penalized
        fclose(imgfs_file->file);
        ret = ERR_IO;
    }
